...

{
  ...
  "Values": {
    ...
    "APIMgmtURL": "<URL>",
    "QualificationsFullCSVExtractURL": "<PATH_TO_FULL_EXTRACT_URL>",
    "QualificationsCSVRecordLimit": 10000
  }
}
...
- `RefdataAPIUrl`: URL for the Ref Data API to fetch filter values for qualifications (qualification types, levels, SSAs, assessment methods, etc.).
...
- `QualificationsCSVRecordLimit`: (Optional) Sets a cut-off point for the number of rows to be fetched from the database when generating the full CSV extract. This is to avoid timeouts and memory issues when generating the CSV. The default value is 10,000 records. 
- `QualificationsFullCSVExtractURL`: (Optional) URL for the full CSV extract of qualifications. this is triggered when the user attempts to download a CSV extract for datasets exceeding the number of rows set in `QualificationsCSVRecordLimit`.
